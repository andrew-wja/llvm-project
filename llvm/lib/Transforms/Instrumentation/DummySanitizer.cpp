//===- DummySanitizer.cpp - detector of uninitialized reads -------===//
//
...
#include "llvm/Transforms/Instrumentation/DummySanitizer.h"
...
#define DEBUG_TYPE "dummysan"

const char kDummysanModuleCtorName[] = "dummysan.module_ctor";
const char kDummysanNoteName[] = "dummysan.note";
const char kDummysanInitName[] = "__dummysan_init";
const char kDummysanPersonalityThunkName[] = "__dummysan_personality_thunk";

const char kDummysanShadowMemoryDynamicAddress[] =
    "__dummysan_shadow_memory_dynamic_address";
...
static cl::opt<std::string> ClMemoryAccessCallbackPrefix(
    "dummysan-memory-access-callback-prefix",
    cl::desc("Prefix for memory access callbacks"), cl::Hidden,
    cl::init("__dummysan_"));

static cl::opt<bool>
    ClInstrumentWithCalls("dummysan-instrument-with-calls",
                cl::desc("instrument reads and writes with callbacks"),
                cl::Hidden, cl::init(false));

static cl::opt<bool> ClInstrumentReads("dummysan-instrument-reads",
                                       cl::desc("instrument read instructions"),
                                       cl::Hidden, cl::init(true));

static cl::opt<bool> ClInstrumentWrites(
    "dummysan-instrument-writes", cl::desc("instrument write instructions"),
    cl::Hidden, cl::init(true));

static cl::opt<bool> ClInstrumentAtomics(
    "dummysan-instrument-atomics",
    cl::desc("instrument atomic instructions (rmw, cmpxchg)"), cl::Hidden,
    cl::init(true));

static cl::opt<bool> ClInstrumentByval("dummysan-instrument-byval",
...
static cl::opt<bool> ClInstrumentStack("dummysan-instrument-stack",
                                       cl::desc("instrument stack (allocas)"),
                                       cl::Hidden, cl::init(true));

static cl::opt<bool> ClUARRetagToZero(
    "dummysan-uar-retag-to-zero",
...
static cl::opt<bool> ClGenerateTagsWithCalls(
    "dummysan-generate-tags-with-calls",
    cl::desc("generate new tags with runtime library calls"), cl::Hidden,
    cl::init(false));

static cl::opt<bool> ClGlobals("dummysan-globals", cl::desc("Instrument globals"),
                               cl::Hidden, cl::init(false), cl::ZeroOrMore);

static cl::opt<int> ClMatchAllTag(
    "dummysan-match-all-tag",
    cl::desc("don't report bad accesses via pointers with this tag"),
    cl::Hidden, cl::init(-1));

static cl::opt<bool> ClEnableKdummysan(
    "dummysan-kernel",
    cl::desc("Enable KernelDummySanitizer instrumentation"),
    cl::Hidden, cl::init(false));
...
static cl::opt<uint64_t>
    ClMappingOffset("dummysan-mapping-offset",
                    cl::desc("Dummysan shadow mapping offset [EXPERIMENTAL]"),
                    cl::Hidden, cl::init(0));

static cl::opt<bool>
    ClWithIfunc("dummysan-with-ifunc",
...
public:
  explicit DummySanitizer(Module &M, bool CompileKernel = false,
                              bool Recover = false) : M(M) {
    this->Recover = ClRecover.getNumOccurrences() > 0 ? ClRecover : Recover;
    this->CompileKernel = ClEnableKdummysan.getNumOccurrences() > 0 ?
        ClEnableKdummysan : CompileKernel;
  ...
  Value *getDummysanThreadSlotPtr(IRBuilder<> &IRB, Type *Ty);
...
private:
  ...
  Module &M;
  Triple TargetTriple;
  FunctionCallee DummysanMemmove, DummysanMemcpy, DummysanMemset;
  FunctionCallee DummysanHandleVfork;

  /// This struct defines the shadow mapping using the rule:
  ///   shadow = (mem >> Scale) + Offset.
  /// If InGlobal is true, then
  ///   extern char __dummysan_shadow[];
  ///   shadow = (mem >> Scale) + &__dummysan_shadow
  /// If InTls is true, then
  ///   extern char *__dummysan_tls;
  ///   shadow = (mem>>Scale) + align_up(__dummysan_shadow, kShadowBaseAlignment)
  ...
  Function *DummysanCtorFunction;

  FunctionCallee DummysanMemoryAccessCallback[2][kNumberOfAccessSizes];
  FunctionCallee DummysanMemoryAccessCallbackSized[2];

  FunctionCallee DummysanTagMemoryFunc;
  FunctionCallee DummysanGenerateTagFunc;
...
class DummySanitizerLegacyPass : public FunctionPass {
public:
  // Pass identification, replacement for typeid.
  static char ID;

  explicit DummySanitizerLegacyPass(bool CompileKernel = false,
                                        bool Recover = false)
      : FunctionPass(ID), CompileKernel(CompileKernel), Recover(Recover) {
    initializeDummySanitizerLegacyPassPass(
        *PassRegistry::getPassRegistry());
  }

  StringRef getPassName() const override { return "DummySanitizer"; }

  bool doInitialization(Module &M) override {
    Dummysan = std::make_unique<DummySanitizer>(M, CompileKernel, Recover);
    return true;
  }

  bool runOnFunction(Function &F) override {
    return Dummysan->sanitizeFunction(F);
  }
...
private:
  std::unique_ptr<DummySanitizer> Dummysan;
  bool CompileKernel;
  bool Recover;
};
...
char DummySanitizerLegacyPass::ID = 0;

INITIALIZE_PASS_BEGIN(
    DummySanitizerLegacyPass, "dummysan",
    "DummySanitizer: a dummy sanitizer.", false,
    false)
INITIALIZE_PASS_END(
    DummySanitizerLegacyPass, "dummysan",
    "DummySanitizer: a dummy sanitizer.", false,
...
FunctionPass *llvm::createDummySanitizerLegacyPassPass(bool CompileKernel,
                                                           bool Recover) {
  assert(!CompileKernel || Recover);
  return new DummySanitizerLegacyPass(CompileKernel, Recover);
}

DummySanitizerPass::DummySanitizerPass(bool CompileKernel, bool Recover)
    : CompileKernel(CompileKernel), Recover(Recover) {}

PreservedAnalyses DummySanitizerPass::run(Module &M,
                                              ModuleAnalysisManager &MAM) {
  DummySanitizer Dummysan(M, CompileKernel, Recover);
  bool Modified = false;
  for (Function &F : M)
    Modified |= Dummysan.sanitizeFunction(F);
  if (Modified)
    return PreservedAnalyses::none();
  return PreservedAnalyses::all();
}

void DummySanitizer::createDummysanCtorComdat() {
  std::tie(DummysanCtorFunction, std::ignore) =
      getOrCreateSanitizerCtorAndInitFunctions(
          M, kDummysanModuleCtorName, kDummysanInitName,
          /*InitArgTypes=*/{},
          /*InitArgs=*/{},
          // This callback is invoked when the functions are created the first
          // time. Hook them into the global ctors list in that case:
          [&](Function *Ctor, FunctionCallee) {
            Comdat *CtorComdat = M.getOrInsertComdat(kDummysanModuleCtorName);
  ...
  //    interposed by A) and triggers a DUMMYSAN failure due to the initialization
  ...
  auto *Note =
      new GlobalVariable(M, NoteTy, /*isConstant=*/true,
                         GlobalValue::PrivateLinkage, nullptr, kDummysanNoteName);
  Note->setSection(".note.dummysan.globals");
  ...
  Note->setInitializer(ConstantStruct::getAnon(
      {ConstantInt::get(Int32Ty, 8),                           // n_namesz
       ...
       ConstantInt::get(Int32Ty, ELF::NT_LLVM_DUMMYSAN_GLOBALS), // n_type
       Name, CreateRelPtr(Start), CreateRelPtr(Stop)}));
  appendToCompilerUsed(M, Note);

  // Create a zero-length global in dummysan_globals so that the linker will
  // always create start and stop symbols.
  auto Dummy = new GlobalVariable(
      M, Int8Arr0Ty, /*isConstantGlobal*/ true, GlobalVariable::PrivateLinkage,
      Constant::getNullValue(Int8Arr0Ty), "dummysan.dummy.global");
  Dummy->setSection("dummysan_globals");
...
///
/// inserts a call to __dummysan_init to the module's constructor list.
void DummySanitizer::initializeModule() {
  ...
  if (!TargetTriple.isAndroid()) {
    Constant *C = M.getOrInsertGlobal("__dummysan_tls", IntptrTy, [&] {
      auto *GV = new GlobalVariable(M, IntptrTy, /*isConstant=*/false,
                                    GlobalValue::ExternalLinkage, nullptr,
                                    "__dummysan_tls", nullptr,
...
void DummySanitizer::initializeCallbacks(Module &M) {
  IRBuilder<> IRB(*C);
  for (size_t AccessIsWrite = 0; AccessIsWrite <= 1; AccessIsWrite++) {
    const std::string TypeStr = AccessIsWrite ? "store" : "load";
    const std::string EndingStr = Recover ? "_noabort" : "";

    DummysanMemoryAccessCallbackSized[AccessIsWrite] = M.getOrInsertFunction(
        ClMemoryAccessCallbackPrefix + TypeStr + "N" + EndingStr,
        FunctionType::get(IRB.getVoidTy(), {IntptrTy, IntptrTy}, false));

    for (size_t AccessSizeIndex = 0; AccessSizeIndex < kNumberOfAccessSizes;
         AccessSizeIndex++) {
      DummysanMemoryAccessCallback[AccessIsWrite][AccessSizeIndex] =
  ...
  DummysanTagMemoryFunc = M.getOrInsertFunction(
      "__dummysan_tag_memory", IRB.getVoidTy(), Int8PtrTy, Int8Ty, IntptrTy);
  DummysanGenerateTagFunc =
      M.getOrInsertFunction("__dummysan_generate_tag", Int8Ty);

  ShadowGlobal = M.getOrInsertGlobal("__dummysan_shadow",
                                     ArrayType::get(IRB.getInt8Ty(), 0));

  const std::string MemIntrinCallbackPrefix =
      CompileKernel ? std::string("") : ClMemoryAccessCallbackPrefix;
  DummysanMemmove = M.getOrInsertFunction(MemIntrinCallbackPrefix + "memmove",
                                        IRB.getInt8PtrTy(), IRB.getInt8PtrTy(),
                                        IRB.getInt8PtrTy(), IntptrTy);
  DummysanMemcpy = M.getOrInsertFunction(MemIntrinCallbackPrefix + "memcpy",
                                       IRB.getInt8PtrTy(), IRB.getInt8PtrTy(),
                                       IRB.getInt8PtrTy(), IntptrTy);
  DummysanMemset = M.getOrInsertFunction(MemIntrinCallbackPrefix + "memset",
                                       IRB.getInt8PtrTy(), IRB.getInt8PtrTy(),
                                       IRB.getInt32Ty(), IntptrTy);
}

Value *DummySanitizer::getOpaqueNoopCast(IRBuilder<> &IRB, Value *Val) {
  ...
  return IRB.CreateCall(Asm, {Val}, ".dummysan.shadow");
}

Value *DummySanitizer::getDynamicShadowIfunc(IRBuilder<> &IRB) {
  return getOpaqueNoopCast(IRB, ShadowGlobal);
}

Value *DummySanitizer::getShadowNonTls(IRBuilder<> &IRB) {
  ...
  if (Mapping.InGlobal) {
    ...
    Value *GlobalDynamicAddress =
        IRB.GetInsertBlock()->getParent()->getParent()->getOrInsertGlobal(
            kDummysanShadowMemoryDynamicAddress, Int8PtrTy);
    return IRB.CreateLoad(Int8PtrTy, GlobalDynamicAddress);
  }
}

bool DummySanitizer::ignoreAccess(Value *Ptr) {
...
void DummySanitizer::getInterestingMemoryOperands(
    Instruction *I, SmallVectorImpl<InterestingMemoryOperand> &Interesting) {
...
void DummySanitizer::untagPointerOperand(Instruction *I, Value *Addr) {
...
Value *DummySanitizer::memToShadow(Value *Mem, IRBuilder<> &IRB) {
  // Mem >> Scale
...
void DummySanitizer::instrumentMemAccessInline(Value *Ptr, bool IsWrite,
                                                   unsigned AccessSizeIndex,
                                                   Instruction *InsertBefore) {
  const int64_t AccessInfo =
      (CompileKernel << DummysanAccessInfo::CompileKernelShift) +
      (HasMatchAllTag << DummysanAccessInfo::HasMatchAllShift) +
      (MatchAllTag << DummysanAccessInfo::MatchAllShift) +
      (Recover << DummysanAccessInfo::RecoverShift) +
      (IsWrite << DummysanAccessInfo::IsWriteShift) +
      (AccessSizeIndex << DummysanAccessInfo::AccessSizeShift);
  ...
  if (OutlinedChecks) {
    ...
    IRB.CreateCall(Intrinsic::getDeclaration(
                       M, UseShortGranules
                              ? Intrinsic::dummysan_check_memaccess_shortgranules
                              : Intrinsic::dummysan_check_memaccess),
  ...
  switch (TargetTriple.getArch()) {
    case Triple::x86_64:
      ...
      Asm = InlineAsm::get(
          ...
          "int3\nnopl " +
              itostr(0x40 + (AccessInfo & DummysanAccessInfo::RuntimeMask)) +
              "(%rax)",
          "{rdi}",
          /*hasSideEffects=*/true);
      break;
    ...
    case Triple::aarch64_be:
      ...
      Asm = InlineAsm::get(
          FunctionType::get(IRB.getVoidTy(), {PtrLong->getType()}, false),
          "brk #" +
              itostr(0x900 + (AccessInfo & DummysanAccessInfo::RuntimeMask)),
          "{x0}",
          /*hasSideEffects=*/true);
      break;
    default:
...
void DummySanitizer::instrumentMemIntrinsic(MemIntrinsic *MI) {
  IRBuilder<> IRB(MI);
  if (isa<MemTransferInst>(MI)) {
    IRB.CreateCall(
        isa<MemMoveInst>(MI) ? DummysanMemmove : DummysanMemcpy,
...
bool DummySanitizer::instrumentMemAccess(InterestingMemoryOperand &O) {
  Value *Addr = O.getPtr();
  ...
  if (isPowerOf2_64(O.TypeSize) &&
    ...
    if (ClInstrumentWithCalls) {
      IRB.CreateCall(DummysanMemoryAccessCallback[O.IsWrite][AccessSizeIndex],
                     IRB.CreatePointerCast(Addr, IntptrTy));
    } else {
      instrumentMemAccessInline(Addr, O.IsWrite, AccessSizeIndex, O.getInsn());
    }
  } else {
    IRB.CreateCall(DummysanMemoryAccessCallbackSized[O.IsWrite],
                   {IRB.CreatePointerCast(Addr, IntptrTy),
...
bool DummySanitizer::tagAlloca(IRBuilder<> &IRB, AllocaInst *AI,
  ...
  if (ClInstrumentWithCalls) {
    IRB.CreateCall(DummysanTagMemoryFunc,
    ...
    // If this memset is not inlined, it will be intercepted in the dummysan
...
Value *DummySanitizer::getNextTagWithCall(IRBuilder<> &IRB) {
  return IRB.CreateZExt(IRB.CreateCall(DummysanGenerateTagFunc), IntptrTy);
}

Value *DummySanitizer::getStackBaseTag(IRBuilder<> &IRB) {
  ...
  Value *StackTag =
      IRB.CreateXor(StackPointerLong, IRB.CreateLShr(StackPointerLong, 20),
                    "dummysan.stack.base.tag");
  return StackTag;
}

Value *DummySanitizer::getAllocaTag(IRBuilder<> &IRB, Value *StackTag,
...
Value *DummySanitizer::getUARTag(IRBuilder<> &IRB, Value *StackTag) {
  if (ClUARRetagToZero)
...
Value *DummySanitizer::tagPointer(IRBuilder<> &IRB, Type *Ty,
...
Value *DummySanitizer::untagPointer(IRBuilder<> &IRB, Value *PtrLong) {
...
Value *DummySanitizer::getDummysanThreadSlotPtr(IRBuilder<> &IRB, Type *Ty) {
  Module *M = IRB.GetInsertBlock()->getParent()->getParent();
...
void DummySanitizer::emitPrologue(IRBuilder<> &IRB, bool WithFrameRecord) {
  if (!Mapping.InTls) {
  ...
  Value *SlotPtr = getDummysanThreadSlotPtr(IRB, IntptrTy);
  assert(SlotPtr);
  ...
  ShadowBase = IRB.CreateAdd(
      ...
      ConstantInt::get(IntptrTy, 1), "dummysan.shadow");
  ShadowBase = IRB.CreateIntToPtr(ShadowBase, Int8PtrTy);
}

Value *DummySanitizer::readRegister(IRBuilder<> &IRB, StringRef Name) {
  Module *M = IRB.GetInsertBlock()->getParent()->getParent();
...
bool DummySanitizer::instrumentStack(
    SmallVectorImpl<AllocaInst *> &Allocas,
    DenseMap<AllocaInst *, std::vector<DbgVariableIntrinsic *>> &AllocaDbgMap,
    SmallVectorImpl<Instruction *> &RetVec, Value *StackTag) {
  ...
  for (unsigned N = 0; N < Allocas.size(); ++N) {
    ...
    Replacement->setName(Name + ".dummysan");
...
bool DummySanitizer::isInterestingAlloca(const AllocaInst &AI) {
...
bool DummySanitizer::sanitizeFunction(Function &F) {
  if (&F == DummysanCtorFunction)
    return false;

  if (!F.hasFnAttribute(Attribute::SanitizeDummy))
    return false;
  ...
  if (AllocasToInstrument.empty() && F.hasPersonalityFn() &&
      F.getPersonalityFn()->getName() == kDummysanPersonalityThunkName) {
    // __dummysan_personality_thunk is a no-op for functions without an
...
void DummySanitizer::instrumentGlobal(GlobalVariable *GV, uint8_t Tag) {
  ...
  for (uint64_t DescriptorPos = 0; DescriptorPos < SizeInBytes;
    ...
    auto *Descriptor =
        new GlobalVariable(M, DescriptorTy, true, GlobalValue::PrivateLinkage,
                           nullptr, GV->getName() + ".dummysan.descriptor");
    ...
    Descriptor->setSection("dummysan_globals");
...
void DummySanitizer::instrumentGlobals() {
...
void DummySanitizer::instrumentPersonalityFunctions() {
  ...
  for (Function &F : M) {
    if (F.isDeclaration() || !F.hasFnAttribute(Attribute::SanitizeDummy))
      continue;
  ...
  FunctionCallee DummysanPersonalityWrapper = M.getOrInsertFunction(
      "__dummysan_personality_wrapper", Int32Ty, Int32Ty, Int32Ty, Int64Ty,
  ...
  for (auto &P : PersonalityFns) {
    std::string ThunkName = kDummysanPersonalityThunkName;
    if (P.first)
...
void DummySanitizer::ShadowMapping::init(Triple &TargetTriple) {
  Scale = kDefaultShadowScale;
  if (ClMappingOffset.getNumOccurrences() > 0) {
    InGlobal = false;
    InTls = false;
    Offset = ClMappingOffset;
  } else if (ClEnableKdummysan || ClInstrumentWithCalls) {
    InGlobal = false;
    InTls = false;
    Offset = 0;
  } else if (ClWithIfunc) {
