...
// See gsan_allocator.h for details.
//
//===----------------------------------------------------------------------===//
...
static void *ReportAllocationSizeTooBig(uptr size, const StackTrace &stack) {
  if (AllocatorMayReturnNull()) {
    Report("WARNING: GenericSanitizer failed to allocate 0x%zx bytes\n", size);
    return nullptr;
  }
...
void *gsan_reallocarray(void *ptr, uptr nmemb, uptr size,
                        const StackTrace &stack) {
  if (UNLIKELY(CheckForCallocOverflow(size, nmemb))) {
    errno = errno_ENOMEM;
    if (AllocatorMayReturnNull())
  ...
  return gsan_realloc(ptr, nmemb * size, stack);
}
...
void GetAdditionalThreadContextPtrs(ThreadContextBase *tctx, void *ptrs) {
  ...
  // This is currently a no-op because the GSan `pthread_create()` interceptor
