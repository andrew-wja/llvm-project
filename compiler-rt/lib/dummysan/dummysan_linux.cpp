...
///
/// \file
/// This file is a part of DummySanitizer and contains Linux-, NetBSD- and
...
THREADLOCAL uptr __dummysan_tls;
#endif
...
#define PR_TAGGED_ADDR_ENABLE (1UL << 0)
  ...
  if (internal_iserror(internal_prctl(PR_GET_TAGGED_ADDR_CTRL, 0, 0, 0, 0),
    ...
    Printf(
        "FATAL: "
        "DummySanitizer requires a kernel with tagged address ABI.\n");
    Die();
#endif
  }
  ...
  if (internal_iserror(internal_prctl(PR_SET_TAGGED_ADDR_CTRL,
    ...
    Printf(
        "FATAL: DummySanitizer failed to enable tagged address syscall "
...
bool InitShadow() {
  ...
  kLowMemEnd = __dummysan_shadow_memory_dynamic_address - 1;
  kLowMemStart = 0;

  // Define the low shadow based on the already placed low memory.
  kLowShadowEnd = MemToShadow(kLowMemEnd);
  kLowShadowStart = __dummysan_shadow_memory_dynamic_address;
...
void InitThreads() {
  CHECK(__dummysan_shadow_memory_dynamic_address);
  uptr guard_page_size = GetMmapGranularity();
  uptr thread_space_start =
      __dummysan_shadow_memory_dynamic_address - (1ULL << kShadowBaseAlignment);
  uptr thread_space_end =
      __dummysan_shadow_memory_dynamic_address - guard_page_size;
  ReserveShadowMemoryRange(thread_space_start, thread_space_end - 1,
                           "dummysan threads", /*madvise_shadow*/ false);
  ProtectGap(thread_space_end,
             __dummysan_shadow_memory_dynamic_address - thread_space_end);
...
static void DummysanAtExit(void) {
  if (common_flags()->print_module_map)
    DumpProcessMap();
  if (flags()->print_stats && (flags()->atexit || dummysan_report_count > 0))
    ReportStats();
  if (dummysan_report_count > 0) {
...
void InstallAtExitHandler() {
  atexit(DummysanAtExit);
}

// ---------------------- TSD ---------------- {{{1

extern "C" void __dummysan_thread_enter() {
  dummysanThreadList().CreateCurrentThread()->InitRandomState();
}

extern "C" void __dummysan_thread_exit() {
  Thread *t = GetCurrentThread();
  // Make sure that signal handler can not see a stale current thread pointer.
  atomic_signal_fence(memory_order_seq_cst);
  if (t)
    dummysanThreadList().ReleaseThread(t);
}
...
Thread *GetCurrentThread() {
  ...
  return dummysanThreadList().GetThreadByBufferAddress((uptr)R->Next());
}
...
static AccessInfo GetAccessInfo(siginfo_t *info, ucontext_t *uc) {
  ...
  // Access type is encoded in the instruction following INT3 as
  // NOP DWORD ptr [EAX + 0x40 + 0xXY]. For Y == 0xF, access size is stored in
  // ESI register. Access address is always in EDI register.
  uptr pc = (uptr)uc->uc_mcontext.gregs[REG_EIP];
  uint8_t *nop = (uint8_t*)pc;
  if (*nop != 0x0f || *(nop + 1) != 0x1f || *(nop + 2) != 0x40  ||
      *(nop + 3) < 0x40)
    return AccessInfo{}; // Not ours.
  ...
  const bool is_store = code & 0x10;
  const bool recover = code & 0x20;
  const uptr addr = uc->uc_mcontext.gregs[REG_EDI];
  const unsigned size_log = code & 0xf;
  if (size_log > 4 && size_log != 0xf)
    return AccessInfo{}; // Not ours.
  const uptr size =
      size_log == 0xf ? uc->uc_mcontext.gregs[REG_ESI] : 1U << size_log;
...
static void HandleTagMismatch(AccessInfo ai, uptr pc, uptr frame,
  ...
  // The second stack frame contains the failure __dummysan_check function, as
  // we have a stack frame for the registers saved in __dummysan_tag_mismatch that
...
static bool DummysanOnSIGTRAP(int signo, siginfo_t *info, ucontext_t *uc) {
...
void DummysanOnDeadlySignal(int signo, void *info, void *context) {
  ...
  if (signo == SIGTRAP)
    if (DummysanOnSIGTRAP(signo, (siginfo_t *)info, (ucontext_t*)context))
      return;
...
// Entry point for interoperability between __dummysan_tag_mismatch (ASM) and the
// rest of the mismatch handling code (C++).
void __dummysan_tag_mismatch4(uptr addr, uptr access_info, uptr *registers_frame,
                            size_t outsize) {
  __dummysan::AccessInfo ai;
  ...
  __dummysan::HandleTagMismatch(ai, (uptr)__builtin_return_address(0),
