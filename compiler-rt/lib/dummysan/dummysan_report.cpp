...
// example, Printf may call syslog() which can itself be built with dummysan).
...
// Dummysan globals store the size of the global in the descriptor. In cases where
...
static uptr GetGlobalSizeFromDescriptor(uptr ptr) {
  ...
  for (const dummysan_global &global :
       DummysanGlobalsFor(load_bias, phdr_begin, ehdr->e_phnum))
...
void PrintAddressDescription(
  ...
  DummysanChunkView chunk = FindHeapChunkByAddress(untagged_addr);
  ...
  if (candidate) {
    ...
    DummysanChunkView chunk = FindHeapChunkByAddress(mem);
  ...
  dummysanThreadList().VisitAllLiveThreads([&](Thread *t) {
    ...
    if (FindHeapAllocation(t->heap_allocations(), tagged_addr, &har,
      ...
      Printf("dummysan_dev_note_heap_rb_distance: %zd %zd\n", ring_index + 1,
             flags()->heap_history_size);
      Printf("dummysan_dev_note_num_matching_addrs: %zd\n", num_matching_addrs);
      Printf("dummysan_dev_note_num_matching_addrs_4b: %zd\n",
             num_matching_addrs_4b);
  ...
  dummysanThreadList().VisitAllLiveThreads([&](Thread *t) { t->Announce(); });

  if (!num_descriptions_printed)
    // We exhausted our possibilities. Bail out.
    Printf("DummySanitizer can not describe address in more detail.\n");
}
...
void ReportTailOverwritten(StackTrace *stack, uptr tagged_addr, uptr orig_size,
  ...
  s.append("\nThis error occurs when a buffer overflow overwrites memory\n"
    ...
    "To disable this feature set DUMMYSAN_OPTIONS=free_checks_tail_magic=0\n",
...
void ReportTagMismatch(StackTrace *stack, uptr tagged_addr, uptr access_size,
  ...
  sptr offset =
      __dummysan_test_shadow(reinterpret_cast<void *>(tagged_addr), access_size);
...
// See the frame breakdown defined in __dummysan_tag_mismatch (from
// dummysan_tag_mismatch_aarch64.S).
...
void __dummysan_set_error_report_callback(void (*callback)(const char *)) {
  __dummysan::ScopedReport::SetErrorReportCallback(callback);
}
