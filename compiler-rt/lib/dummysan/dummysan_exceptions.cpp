...
__dummysan_personality_wrapper(int version, _Unwind_Action actions,
                             uint64_t exception_class,
                             _Unwind_Exception* unwind_exception,
                             _Unwind_Context* context,
  ...
  // required by AAPCS but is a requirement for DUMMYSAN instrumented functions.
  if ((actions & _UA_CLEANUP_PHASE) && rc == _URC_CONTINUE_UNWIND) {
#if defined(__x86_64__) || defined(__i386__)
    uptr fp = get_gr(context, 6); // rbp on x86_64 and ebp on i386
