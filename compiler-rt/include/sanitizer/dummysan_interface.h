//===-- sanitizer/dummysan_interface.h ------------------------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
#ifndef SANITIZER_DUMMYSAN_INTERFACE_H
#define SANITIZER_DUMMYSAN_INTERFACE_H
...
extern "C" {
  ...
  void __dummysan_init_static(void);

  // This function may be optionally provided by user and should return
  // a string containing DummySan runtime options. See asan_flags.h for details.
  const char* __dummysan_default_options(void);

  void __dummysan_enable_allocator_tagging(void);
  void __dummysan_disable_allocator_tagging(void);

  // Mark region of memory with the given tag. Both address and size need to be
  // 16-byte aligned.
  void __dummysan_tag_memory(const volatile void *p, unsigned char tag,
                           size_t size);

  /// Set pointer tag. Previous tag is lost.
  void *__dummysan_tag_pointer(const volatile void *p, unsigned char tag);
  ...
  void __dummysan_handle_vfork(const void *sp_dst);
  ...
  // format.
  void __dummysan_print_shadow(const volatile void *x, size_t size);
  ...
  intptr_t __dummysan_test_shadow(const volatile void *x, size_t size);

  /* Sets the callback function to be called during DummySan error reporting. */
...
#endif  // SANITIZER_DUMMYSAN_INTERFACE_H
